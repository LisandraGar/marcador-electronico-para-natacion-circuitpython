...
class MQTT:  # noqa: PLR0904  # too-many-public-methods
    ...
    def loop(self) -> Optional[list[int]]:
        ...
        Busca el socket dinámicamente para no dar error de atributo.
        """
        self._connected()
        rcs = []
        
        # 1. Identificar el socket (Intentamos los nombres comunes)
        # Esto evita el error "objeto MQTT no tiene atributo X"
        sock = getattr(self, "sock", getattr(self, "_sock", getattr(self, "socket", None)))
        
        if sock is None:
            # Si no encontramos el socket, usamos el método estándar pero con timeout 0
            # para que no bloquee, aunque es menos eficiente que select.poll
            return self._wait_for_msg()

        # 2. Gestión de KeepAlive (Solo envía, no espera respuesta)
        if ticks_diff(ticks_ms(), self._last_msg_sent_timestamp) / 1000 >= self.keep_alive:
            # En lugar de self.ping() que bloquea esperando respuesta,
            # llamamos al método interno que solo envía el paquete.
            if hasattr(self, "_send_pingreq"):
                self._send_pingreq()
                self._last_msg_sent_timestamp = ticks_ms()
            else:
                # Si no existe _send_pingreq, usamos ping() pero solo cuando es estrictamente necesario
                rcs.extend(self.ping())

        # 3. La clave para que NO BLOQUEE: select.poll
        poller = select.poll()
        poller.register(sock, select.POLLIN)
        
        # Si poll(0) devuelve algo, hay datos. Si no, salimos en microsegundos.
        if poller.poll(0):
            try:
                rc = self._wait_for_msg()
                if rc is not None:
